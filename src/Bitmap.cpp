...
Bitmap::Matrix::Matrix(const char *path, uint32_t offset, uint32_t imageSize,
                       uint32_t width, uint32_t height, uint32_t padding)
    : pix{new char[imageSize]}, mWidth{width}, mHeight{height}, mPadding{padding}
{
    std::ifstream ifs(path, std::ios::binary);
    ifs.seekg(offset);
    ifs.read(pix, imageSize);
}
Bitmap::Matrix::Matrix(uint32_t imageSize, uint32_t width, uint32_t height,
                       uint32_t padding)
    : pix{new char[imageSize]}, mWidth{width}, mHeight{height}, mPadding{padding}
{
}
char *Bitmap::Matrix::operator()(uint32_t x, uint32_t y)
{
...
Bitmap::Bitmap(const char *path)
    : fileHeader(path), dibHeader(path, Bitmap::FileHeader::FILE_HEADER_SIZE),
      matrix(path, fileHeader.bfOffsetBits, dibHeader.biSizeImage, dibHeader.biWidth,
             dibHeader.biHeight, (4 - ((dibHeader.biWidth * BYTE_PER_PIX) & 0x3)) & 0x3),
...
void Bitmap::Rotate90()
{
    ...
    uint32_t desSize{(dibHeader.biWidth * BYTE_PER_PIX + newPadding) *
                     dibHeader.biHeight};
    uint32_t difference{
        desSize -
        dibHeader
            .biSizeImage}; // 这里不必考虑desSize与dibHeader.biSizeImage孰大孰小的问题，因为无符号整数的溢出是回绕的
